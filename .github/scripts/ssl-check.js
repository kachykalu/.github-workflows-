'use strict';
  ...
  function sslHostFromUrl(rawUrl) {
    try {
      ...
      const defaultPort = url.protocol === 'https:' ? '443' : url.protocol
  === 'http:' ? '80' : '';
  ...
  async function loadHosts(workerUrl) {
    try {
      ...
      console.warn(`Could not load dynamic app hosts (${e.message}); using
  defaults.`);
  ...
  function checkHost(host) {
    ...
    const port     = host.includes(':') ? parseInt(host.split(':').pop(),
  10) : 443;
    const out      = { host, valid: false, issuer: '—', subject: '—',
  expires: '—', daysLeft: null, state: 'unknown', error: null };
  ...
  async function main() {
    ...
    for (const r of results) {
      const icon = r.state === 'ok' ? '✅' : r.state === 'warn' ? '⚠️ ' :
  r.state === 'critical' ? '🔴' : '❌';
  ...
  `Bearer ${token}` },
    ...
    if (!res.ok) {
      console.error(`Worker rejected the payload: ${res.status} ${await
  res.text()}`);
      process.exit(1);
    }
